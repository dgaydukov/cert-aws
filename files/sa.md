...
* MultiFactorAuthPresent (existence) - if user used 2FA. The key is only present when the user authenticates with short-term credentials, for long-term this key is not present.
* MultiFactorAuthAge (duration) - numeric value how long ago short-term credentials with 2FA were created. If credentials were created without 2FA, value is null.
...
These 2 verify that you used mfa code `"Condition":{"Null":{"aws:MultiFactorAuthAge":"false"}}` (allow when 2FA is present) == `"Condition": {"Bool": {"aws:MultiFactorAuthPresent": "true"}}` with allow policy.
Alternatively you can add deny policy with `{ "Null": { "aws:MultiFactorAuthAge": true } }` - deny when 2FA not present
