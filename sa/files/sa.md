...
* automatic backup - snapshots are taken by RDS daily, retained for limited period (by default 7 days). First snapshot contains full db instance, subsequent - incremental taking only what has been changed.
...
* PG (Parameter Group) - a list of db config values (db engine configuration) that can be applied to 1 or many rds instances (for example you can increase a number of connections to rds here). 
...
You use temporary tokens generated by `aws rds generate-db-auth-token` command, and use this token and to access your db (token would be valid for 15 min).
...
mysql -u ssl_user -p'password' -h {RDS_ENDPOINT} -D mydb
