...
When new kms feature avaialable, you have to manually add them to existing key policy, if you want to use them (until you do these, if you just add these features from iam, it won't work)
...
* sign Cluster CSR (after you add first HSM, you must sign cluster CSR). Download `Download certificate signing request` and rename it to `cluster.scr`. For prod aws recommend to use offsite/offline HSM to create private key.
```
